...
Все запуски выполняются через утверждённый remote VM/CI workflow. Локальный
запуск Molecule и ручное исправление тестовой машины не используются.

| Симптом | Причина | Решение |
|---------|---------|---------|
| Converge падает на отсутствующей группе | Тестовая конфигурация ссылается на системную группу, которой нет в базовом образе | Декларативно создать требуемую тестовую группу в `prepare.yml` |
| Converge падает на отсутствующем каталоге или бинарнике внешнего компонента | Роль настраивает уже установленный компонент, но тестовый образ не предоставляет предпосылку | Установить предпосылку в `prepare.yml`; не переносить установку в чужую роль |
| После изменения sudoers следующие задачи теряют `become` | Новая групповая sudo-политика перекрыла NOPASSWD-правило пользователя автоматизации | Добавить узкое test-only NOPASSWD-правило для automation user с более поздним порядком загрузки |
| Условие ведёт себя как строка вместо boolean | Jinja-фильтр вернул текстовое значение, которое затем интерпретируется как truthy | Явно преобразовать результат через `| bool` в месте принятия решения |
| Сценарная переменная не применяется | `vars_files` или extra vars имеют более высокий приоритет | Убрать дублирующий источник и хранить решение на одном предсказуемом уровне precedence |
| Idempotence показывает изменения | Задача не сошлась к стабильному состоянию | Исправить задачу локально и повторить полный workflow на свежем одноразовом окружении; не скрывать изменение через `changed_when: false` |
