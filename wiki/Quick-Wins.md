...
Роль управляет `/etc/docker/daemon.json` и состоянием Docker service. Пакет Docker
устанавливается общим package layer. Роль не добавляет пользователей в группу
`docker`, потому что доступ к Docker socket эквивалентен root-доступу.

**Основные настройки в `defaults/main.yml`:**
...
docker_icc: "{{ true if 'developer' in (workstation_profiles | default([])) else false }}"
docker_live_restore: true
docker_no_new_privileges: true
docker_log_driver: "{{ 'journald' if ansible_facts['service_mgr'] == 'systemd' else 'local' }}"
...
На non-systemd host роль использует `local`; для него рендерятся `max-size` и
`max-file`. Эти rotation options также применяются к явно выбранному `json-file`.
Дополнительные daemon keys передаются через `docker_daemon_extra`; ключи, уже
принадлежащие отдельным переменным роли, там запрещены.

### Файлы

- `ansible/roles/docker/defaults/main.yml` — публичный контракт переменных
- `ansible/roles/docker/templates/daemon.json.j2` — формирование daemon config
- `ansible/roles/docker/tasks/configure/daemon.yml` — валидировать и развернуть daemon config
- `ansible/roles/docker/tasks/service.yml` — применить изменённый config и обеспечить service state
...
sudo docker info | grep "Security Options"
# Должно быть: userns
...
sudo docker network inspect bridge | grep "com.docker.network.bridge.enable_icc"
# false

# Проверка no-new-privileges
sudo docker run --rm alpine cat /proc/self/status | grep NoNewPrivs
# NoNewPrivs: 1
```
...
- **ICC disabled вне developer profile**: default bridge закрыт; user-defined networks продолжают работать
