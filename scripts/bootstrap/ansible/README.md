...
- **task test** - запускает Molecule тесты:
  - Применяет роль на localhost
  - Проверяет идемпотентность (повторный запуск не должен менять систему)
  - Валидирует результаты (установлены пакеты, сервисы запущены, конфиги корректны)
