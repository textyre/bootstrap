...
6. **Report** — calls `common/report_phase.yml` for each phase (install + verify) and `common/report_render.yml` to emit a structured execution table (skipped via `--skip-tags report` in CI)
