...
|----------|---------|--------|-------------|
| `git_owner` | `{name: SUDO_USER}` | safe | Primary user dict (see structure below) |
| `git_additional_users` | `[]` | safe | List of additional user dicts |
...
| Variable | Default | Safety | Description |
|----------|---------|--------|-------------|
| `git_default_branch` | `main` | safe | `init.defaultBranch` |
| `git_editor` | `vim` | safe | `core.editor` |
| `git_pull_rebase` | `true` | safe | `pull.rebase` |
| `git_push_autosetup_remote` | `true` | safe | `push.autoSetupRemote` |
| `git_core_autocrlf` | `input` | safe | `core.autocrlf` |
| `git_commit_sign` | profile-aware | careful | `commit.gpgSign` -- `true` if `security` in `workstation_profiles`. Enabling without a valid signing key causes commit failures. |
| `git_credential_helper` | `cache --timeout=3600` | safe | Shared credential helper |
| `git_safe_directories` | `[]` | safe | `safe.directory` entries (CVE-2022-24765) |

### Subsystem toggles (ROLE-010)

| Variable | Default | Safety | Description |
|----------|---------|--------|-------------|
| `git_manage_signing` | profile-aware | careful | Enable signing config -- `true` if `developer` or `security` profile. Enabling without `signing_key` set fails the role. |
| `git_manage_aliases` | `true` | safe | Enable alias management |
| `git_manage_credential` | `true` | safe | Enable credential helper |
| `git_manage_hooks` | `false` | careful | Enable global hooks directory -- changing affects all repos for the user |
| `git_lfs_enabled` | `true` | safe | Install and init git-lfs |

### Aliases

| Variable | Default | Safety | Description |
|----------|---------|--------|-------------|
| `git_aliases_preset` | `st`, `co`, `br`, `ci`, `lg`, `unstage`, `last`, `amend` | safe | Built-in aliases |
| `git_aliases_extra` | `{}` | safe | Additional aliases (merged on top of preset) |
| `git_aliases_overwrite` | `{}` | safe | Override any alias (highest priority) |

### Extra config

| Variable | Default | Safety | Description |
|----------|---------|--------|-------------|
| `git_config_extra` | `{}` | safe | Arbitrary git config dict (`key: value`) |
| `git_config_overwrite` | `{}` | careful | Override extra config (highest priority) -- can overwrite preset values |
| `git_hooks_path` | `~/.config/git/hooks` | careful | Global hooks directory path -- changing affects all repos |

### Internal mappings (`vars/`)

| File | What it contains | When to edit |
|------|-----------------|-------------|
| `vars/archlinux.yml` | Arch package names (`git`, `git-lfs`) | Only when Arch package names change |
| `vars/debian.yml` | Debian/Ubuntu package names (`git`, `git-lfs`) | Only when Debian package names change |
| `vars/redhat.yml` | RedHat/Fedora package names (`git`, `git-lfs`) | Only when RedHat package names change |
| `vars/void.yml` | Void Linux package names (`git`, `git-lfs`) | Only when Void package names change |
| `vars/gentoo.yml` | Gentoo package names (`dev-vcs/git`, `dev-vcs/git-lfs`) | Only when Gentoo package names change |
...
```yaml
# In group_vars/all/git.yml or host_vars/<hostname>/git.yml:
git_owner:
  name: textyre
  user_name: "textyre"
...
- `name` -- system username that owns the gitconfig file
- `signing_method: none` -- no commit signing. Other options: `ssh` (requires git 2.34+) or `gpg`
- `credential_helper: ""` -- uses the shared default (`cache --timeout=3600`). Set per-user to override.

### SSH signing

```yaml
# In host_vars/<hostname>/git.yml:
git_owner:
  name: textyre
  user_name: "textyre"
  user_email: "textyre@example.com"
...
- `signing_method: ssh` -- use SSH keys for signing (requires git 2.34+). Alternative: `gpg` for GPG signing
- `signing_key` -- path to the SSH public key used for signing. For GPG, use the key ID instead
- `git_commit_sign: true` -- enable automatic signing on every commit. Without this, signing must be done manually with `git commit -S`
- `git_manage_signing: true` -- enable the signing subsystem. Without this, signing config is skipped entirely
...
- `git_additional_users` -- list of user dicts following the same structure as `git_owner`. Each user gets independent gitconfig.
- `credential_helper: "store"` -- stores credentials in plaintext `~/.git-credentials`. Use only on trusted machines. Default `cache` is safer.
- `git_config_extra` -- arbitrary key-value pairs applied via `git config --global`. Keys are git config paths.
- `git_manage_hooks: true` -- creates `~/.config/git/hooks` and sets `core.hooksPath`. All repos for the user will use this hooks directory.
- `git_safe_directories` -- marks directories as safe for git operations (CVE-2022-24765). Required for repos owned by different users.
...
|--------|-----------|---------------|---------------|------------|--------|
| Git package | `git` | `git` | `git` | `git` | `dev-vcs/git` |
| LFS package | `git-lfs` | `git-lfs` | `git-lfs` | `git-lfs` | `dev-vcs/git-lfs` |
| Config path | `~/.gitconfig` | `~/.gitconfig` | `~/.gitconfig` | `~/.gitconfig` | `~/.gitconfig` |
...
| `docker` | Docker | Arch Linux + Ubuntu 24.04 (systemd containers) | `root` |
...
# Docker scenario (requires Docker + Arch/Ubuntu systemd images)
...
- Verify step: all 22 assertions pass
...
|----------|----------|-------------|
| Packages | `git` installed via `package_facts`, `git --version` returns valid output | TEST-008 |
| Config files | `~/.gitconfig` exists, contains `[init]`, `[core]`, `[pull]` sections | TEST-008 |
| Config values | `user.name`, `user.email`, `defaultBranch`, `editor`, `pull.rebase`, `push.autoSetupRemote`, `core.autocrlf` match expected | TEST-008 |
| Extra config | Arbitrary key-value pairs from `git_config_extra` present in gitconfig | TEST-008 |
| Safe directories | `safe.directory` entries present in gitconfig | TEST-008 |
| Aliases | All merged aliases (preset + extra + overwrite) present and correct | TEST-008 |
| Credential | `credential.helper` starts with `cache` | TEST-008 |
| LFS | `git lfs version` works, `filter.lfs.clean/smudge/process` configured | TEST-008 |
| Hooks | Hooks directory exists, `core.hooksPath` matches expected | TEST-008 |
| Signing | `commit.gpgSign` matches expected when signing enabled | TEST-008 |

### Common test failures

| Error | Cause | Fix |
|-------|-------|-----|
| `git package not found in installed packages` | Package installation failed or package name mismatch | Check `vars/<os_family>.yml` has correct package name for the platform |
| `git user.name mismatch` | Another tool overwrites `~/.gitconfig` | Check for chezmoi, dotfiles, or other config managers that write gitconfig |
| `filter.lfs.clean not found in user gitconfig` | `git lfs install --skip-repo` did not run | Ensure `git_lfs_enabled: true` and the git-lfs binary is in PATH |
| `credential.helper mismatch: expected value starting with 'cache'` | Per-user `credential_helper` overrides shared default | Check `git_owner.credential_helper` value -- empty string uses shared default |
| `Global hooks directory does not exist or is not a directory` | `git_manage_hooks` was enabled but directory creation failed | Check filesystem permissions for `git_hooks_path` |
| Idempotence failure (`changed != 0`) | A task is not idempotent (e.g., safe.directory duplicates) | Check `git config --global --get-all safe.directory` for duplicate entries |
| `Assert supported operating system` fails | Running on unsupported OS | Add OS to `git_supported_os` in defaults or use a supported platform |
...
molecule/
  ...
  default/
    ...
    converge.yml    # Stub: imports shared/converge.yml
    verify.yml      # Stub: imports shared/verify.yml
  docker/
    molecule.yml    # Docker driver (Arch + Ubuntu systemd)
...
|-----|-------------|----------|
| `git` | Entire role | Full git setup or reconfiguration |
| `git`, `install` | Package installation only | Re-install git packages without reconfiguring |
| `git`, `configure` | All per-user configuration | Re-apply config after inventory changes without reinstalling |
| `git`, `signing` | Commit signing only | Update signing config after key rotation |
| `git`, `hooks` | Global hooks directory only | Set up hooks directory without full reconfiguration |
| `git`, `lfs` | Git LFS only | Re-initialize LFS after package update |
| `git`, `verify` | In-role verification only | Check current state without making changes |
...
# Only reconfigure signing after key rotation
ansible-playbook playbook.yml --tags "git,signing"

# Verify current state without changes
ansible-playbook playbook.yml --tags "git,verify"
```
...
| `requirements.yml` | Molecule dependency declaration for `common` role | When role dependencies change |
