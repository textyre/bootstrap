...
Причина: captree — системная утилита Arch Linux, скомпилированная с Go 1.25.3.
  ignore-unfixed: true НЕ пропускает эти CVE потому что они FIXED (просто
  пакет ещё не пересобран апстримом с новым Go).
  OS-пакеты чистые (0 уязвимостей).

Фикс: добавить vuln-type: os к gate-шагу
  Логика: base-образ для Ansible/Molecule тестирования — сканируем OS-пакеты.
  gobinary CVEs от апстрима попадают в SBOM (Trivy SBOM-шаг сканирует всё).
```

**Урок:** `ignore-unfixed: true` пропускает только те CVE, для которых нет fix-версии. CVE со status=fixed (но не установленной) всё равно блокируют. Для базовых OS-образов `vuln-type: os` правильнее для gate.
