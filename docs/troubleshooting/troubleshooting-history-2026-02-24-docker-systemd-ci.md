...
| Сервис в Docker требует firewall/netfilter runtime | Контейнер не является полноценной host networking средой: kernel firewall capabilities, netfilter state и service runtime могут отличаться от VM/bare-metal | Docker scenario считать limited install/config/idempotence проверкой; runtime proof переносить в VM/Vagrant или bare-metal scenario |
